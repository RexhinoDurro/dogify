...
const useBotDetection = () => {
  ...
  // Check with server-side detection
  const checkWithServer = async () => {
    try {
      ...
      console.error('Server detection check failed:', error);
    }
    return null;
  };

  // Run detection periodically and check with server
  useEffect(() => {
    const initialDetection = setTimeout(async () => {
      const clientResult = await runDetection();
      const serverResult = await checkWithServer();
      
      if (serverResult && serverResult.blocked) {
        // Server says this is a bot - update our state
        setDetectionResult(prev => ({
          ...prev,
          isBot: true,
          confidence: Math.max(prev.confidence, serverResult.confidence || 0.8),
          detectionMethods: [...prev.detectionMethods, 'server_detection']
        }));
      }
    }, 2000);

    const periodicDetection = setInterval(async () => {
      await runDetection();
      // Optional: periodic server checks
    }, 10000);
...
const BotProtection: React.FC<BotProtectionProps> = ({ children, onBotDetected, strictMode = false }) => {
  ...
  useEffect(() => {
    ...
    } else if (detectionResult.confidence < 0.3 && behaviorMetrics.timeSpent > 5000) {
      // Human detected after sufficient analysis time - redirect
      if (!redirecting) {
        setRedirecting(true);
        setTimeout(() => {
          window.location.href = "http://localhost:5173"; // Your main site
        }, 3000);
      }
    }
  }, [detectionResult, strictMode, onBotDetected, behaviorMetrics.timeSpent, redirecting]);
  ...
  const reportBotDetection = async (result: DetectionResult) => {
    try {
      ...
      if (!response.ok) {
        console.warn('Failed to report bot detection:', response.status);
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-500 to-pink-600 flex items-center justify-center">
        <div className="max-w-2xl mx-auto text-center p-8 bg-white/10 backdrop-blur-lg rounded-3xl border border-white/20">
          ...
          <p className="text-xl text-white/90 mb-8">
            This site is protected against automated access. If you're human, please try again later.
          </p>
          ...
          <div className="text-center">
            <div className="mb-12">
              ...
              <p className="text-xl md:text-2xl text-white/90 mb-8 max-w-3xl mx-auto leading-relaxed">
                Advanced bot protection system. Humans are automatically redirected to the main site.
                Bots and automated systems are blocked for security.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-12">
              <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 hover:bg-white/15 transition-colors">
                ...
                <p className="text-white/80">
                  Advanced client and server-side detection with behavioral analysis
                </p>
              </div>
              
              <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 hover:bg-white/15 transition-colors">
                <div className="text-4xl mb-4">👤</div>
                <h3 className="text-white font-bold text-xl mb-4">Human Verification</h3>
                <p className="text-white/80">
                  Verified humans are automatically redirected to the main website
                </p>
              </div>
              
              <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 border border-white/20 hover:bg-white/15 transition-colors">
                <div className="text-4xl mb-4">🚫</div>
                <h3 className="text-white font-bold text-xl mb-4">Bot Blocking</h3>
                <p className="text-white/80">
                  Automated systems and bots are blocked to protect the main site
                </p>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 max-w-2xl mx-auto border border-white/20">
              ...
              <p className="text-white/90 mb-6">
                Interact with this page naturally to verify you're human.
                The system will automatically redirect you once verified.
              </p>
            ...
            <div className="mt-12">
              <p className="text-white/70 text-lg">
                ...
                🌐 Humans redirected automatically • 
